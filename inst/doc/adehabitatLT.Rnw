...
between animals (Richard et al. in prep). This approach can
...
\item Note that for multiple null models, two kinds of constraints are
  ...
  Richard et al., in prep for an example). The two types of constraint
...
\item Richard, E., Calenge, C., Said, S., Hamann, J.L. and Gaillard,
  J.M. (in prep.) A null model approach to study the spatial
  interactions between roe deer (\textit{Capreolus capreolus}) and red
  deer (\textit{Cervus elaphus}).
